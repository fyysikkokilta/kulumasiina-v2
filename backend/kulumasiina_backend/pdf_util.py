...
def generate_combined_pdf(data: FancyType) -> bytes:
  ...
  attachements = [
    liite for part in data["parts"] for liite in part["liitteet"]
  ]
  i = 1
  ...
  table_data: list[tuple[str,str,str]] = [
    ("Selite", "Liitteet", "Hinta")
  ]
  for part in data["parts"]:
    table_data.append((part["selite"], ", ".join(part["liitteet"]), part["hinta"],))
  ...
  for i, attachement in enumerate(attachements):
    ...
    if fileformat in {"JPG", "PNG", "GIF"}:
      ...
      new_pdf.add_page()
      new_pdf.image(attachement, x=0, y=0, w=210, h=297, alt_text=f"Liite {i+1}", keep_aspect_ratio=True)
