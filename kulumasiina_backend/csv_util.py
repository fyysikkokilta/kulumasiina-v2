...
def merge_csv_infos(csv_infos: list[CsvInfo]) -> list[CsvInfo]:
    ...
    for csv_info in csv_infos:
        ...
        for merged_csv_info in merged_csv_infos:
            same_iban = removeAllWhitespace(
                merged_csv_info["IBAN"]
            ) == removeAllWhitespace(csv_info["IBAN"])
            same_hetu = removeAllWhitespace(
                merged_csv_info["HETU"] or ""
            ) == removeAllWhitespace(csv_info["HETU"] or "")
            if same_iban and same_hetu:
                ...
                merged_csv_info["entry_id"] = (
                    f"{merged_csv_info['entry_id']}-{csv_info['entry_id']}"
                )
                found = True
                break
        if not found:
        ...
        if hasExpenses(rows):
            ...
            for row in filter(isExpense, rows):
                ...
                writer.writerow(
                    [
                        "",
                        substring80AndRemoveNewlines(row["selite"]),
                        "",
                        row["maara"],
                        "kpl",
                        row["yksikkohinta"],
            ...
            for row in filter(isMileage, rows):
                ...
                writer.writerow(
                    [
                        "",
                        substring80AndRemoveNewlines(row["selite"]),
                        os.environ["MILEAGE_PROCOUNTOR_PRODUCT_ID"],
                        row["maara"],
                        "km",
                        row["yksikkohinta"],
                        0,
