...
test('home page loads with expense form', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'FK-Expenses', level: 1 })).toBeVisible()
...
test('login page shows Google login option', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Login with Google' })).toBeVisible()
})
...
test('language switcher toggles to Finnish', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'FK-Kulut', level: 1 })).toBeVisible()
})

test('language switcher toggles back to English from Finnish', async ({ page }) => {
  await page.goto('/fi')
